...
import com.algashop.ordering.infrastructure.persistence.order.OrderPersistenceEntityAssembler;
...
import com.algashop.ordering.infrastructure.persistence.order.OrdersPersistenceProvider;
...
class OrdersIT {
    ...
    public void shouldReturnSalesQuantityByCustomer() {
        ...
        Assertions.assertThat(
                orders.salesQuantityByCustomerInYear(customerId, Year.now())
        ).isEqualTo(2L);

        Assertions.assertThat(
                orders.salesQuantityByCustomerInYear(
                        customerId, Year.now().minusYears(1))
        ).isZero();
